...
로이 필딩 (Roy Fielding), 2009  
HTTP의 주요 저자 중 한 사람으로 웹(HTTP) 설계의 우수성을 십분 활용하고자 구상한 아키텍처.  
...
통일성 있고 한정적인 인터페이스로 수행하는 아키텍처 스타일을 말합니다. (구체적으로?)

&nbsp;  

2) Stateless (무상태성)
세션, 쿠키 등 작업을 위한 상태정보를 따로 저장하고 관리하지 않습니다.   
들어오는 요청만 처리하면 됩니다. 
서비스의 자유도가 높아지고 불필요한 정보를 관리하지 않아서 구현이 단순해집니다.

&nbsp;  

3) Cacheable (캐시 가능)
REST는 HTTP라는 기존 웹표준을 그대로 사용하기 때문에,   
웹에서 사용하는 기존 인프라를 그대로 활용이 가능합니다.   
그 예로 HTTP가 가진 캐싱 기능이 적용 가능합니다.   
